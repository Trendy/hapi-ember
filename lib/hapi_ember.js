...
internals.responseHandler =  function responseHandler(request, reply){
    ...
    // internals.setRoutingTable(internals.server.table());
    if(!response.isBoom){
    ...
    } else if(internals.matchesEmberRoute(request)){
...
internals.matchesEmberRoute = function matchesEmberRoute(request){
    return (internals.server.match(getMethod(request), getPath(request)).path === internals.options.path);
};
